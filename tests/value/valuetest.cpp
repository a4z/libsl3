  ...
  GIVEN ("a check throw on invalid value access function")
  {
    ...
    WHEN ("trying to access any value")
    {
      ...
      THEN ("err null values access will be throws")
      {
        ...
        // add further investigation if I do the case operators as they should be or, if they should be removed
        // CHECK_THROWS_AS((void)static_cast<sl3::Blob>(val), sl3::ErrNullValueAccess) ;
        CHECK_THROWS_AS((void)static_cast<const sl3::Blob&>(val), sl3::ErrNullValueAccess) ;
      }
    }
  }
}
      ...
      if(val.getType() != Type::Blob)
        {
          ...
          CHECK_THROWS_AS((void)static_cast<const Blob&>(val), ErrTypeMisMatch) ;
        }
      else
        {
